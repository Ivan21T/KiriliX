document.addEventListener("DOMContentLoaded", function() {
    ...
    if (!email) {
        showAlert('Няма имейл за възстановяване. Моля, опитайте отново.', 'error');
        setTimeout(() => {
    ...
    resendBtn.addEventListener('click', async function() {
        if (!this.disabled) {
            try {
                ...
                const apiUrl = `${window.API_CONFIG?.USER}/resend?email=${encodeURIComponent(email)}&offsetTime=${offset}`;
    ...
    document.getElementById('activationForm').addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (response.ok) {
                showAlert(result.message || 'Кодът е валиден! Пренасочваме към страницата за нова парола...', 'success');
    ...
    window.addEventListener('load', () => {
        ...
        setTimeout(() => {
            showAlert('Кодът е изпратен на вашия имейл. Моля, проверете пощата си.', 'success');
        }, 1000);
    });
});